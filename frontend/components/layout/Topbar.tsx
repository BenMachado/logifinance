...
export function Topbar({ periodLabel }: { periodLabel: string }) {
  ...
  return (
    <header className="relative sticky top-0 z-10 flex h-16 w-full items-center justify-between border-b border-white/5 bg-black/60 backdrop-blur-md px-md md:px-margin">
      ...
      <div className="flex items-center gap-md">
        {/* Período */}
        <label className="hidden items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-[13px] text-white lg:flex hover:bg-white/10 transition-colors cursor-pointer backdrop-blur-sm">
          <span className="material-symbols-outlined text-[16px] text-[#888]">calendar_month</span>
          <select
            className="bg-transparent text-[13px] font-medium text-white outline-none cursor-pointer"
            defaultValue={periodLabel}
            aria-label="Periodo"
          >
            <option value={periodLabel} className="bg-[#111] text-white">{periodLabel}</option>
            <option className="bg-[#111] text-white">Últimos 14 dias</option>
            <option className="bg-[#111] text-white">Últimos 30 dias</option>
          </select>
        </label>
        ...
        <div
          className={cn(
            ...
            "bg-[hsl(226,71%,40%)] font-display text-sm font-extrabold text-white border border-white/20 shadow-sm",
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="absolute left-0 right-0 top-16 z-30 border-b border-white/10 bg-[#0a0a0a]/95 backdrop-blur-md p-md shadow-card md:hidden">
          {NAV_GROUPS.map((group) => (
            <div key={group.label} className="mb-md">
              <p className="mb-xs px-3 font-mono text-label-caps uppercase font-semibold text-[#888]">
                {group.label}
              </p>
